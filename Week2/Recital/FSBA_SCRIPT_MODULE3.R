...
# An even better one! (See Addendum A)
...
## Addendum ##
##############
## The Bubble Charts in the last 2 examples can be made more rigorous.
## See the comment in the Week 2 Forum: "Circle Definitions and Area in HR Bubble Chart"
## https://www.coursera.org/learn/strategic-business-analytics/discussions/weeks/2

#######
## A ##
#######
## Human perceive area of shapes like circles. So if some value is twice is large,
## we want the area to be twice as large, not the radius. The symbols() function
## takes the radius of the circles so we must calculate this from the desired area.

## Example ##
op <- par(mfrow = c(1,2))
## (Incorrect)
size <- c(1, 2)
symbols(x = c(1,2), y = c(1,1), circles = size,
        inches = 0.75, bg = "blue", main = "Incorrect",
        xlim = c(0.5,2.5))
## The first circle (area 1) should fit inside the second circle (area 2) exactly
## 2 times but it looks like it would fit 4 times.  That's because the radius is squared.

## (Correct)
size <- c(1, 2)
radius <- sqrt(size / pi)               # Area = radius^2 * pi
symbols(x = c(1,2), y = c(1,1), circles = radius,
        inches = 0.75, bg = "blue", main = "Correct",
        xlim = c(0.5,2.5))
...
## Create 20 bins of equal size between 0 and 1 and assign each
...
radius <- sqrt(area / pi)
symbols(aggbSatisRank$rankSatis,aggbSatisRank$left,circles=radius, inches=.2, fg="white", bg="red",main= "Satisfaction and Employee Attrition", ylab="Average Attrition Rate", xlab= "Rank of Satisfaction")
